  ...
  return (
    <nav
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
        scrolled ? 'bg-charcoal-950/95 backdrop-blur-sm border-b border-charcoal-800' : 'bg-transparent'
      }`}
    >
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="flex h-20 items-center justify-between">
          <a href="#" className="flex items-center font-['DM_Sans'] text-xl font-bold tracking-wider">
          ...
          <div className="hidden items-center gap-8 lg:flex">
            {navLinks.map((link) => (
              <a
                key={link.key}
                href={link.href}
                className="text-sm font-medium text-silver-400 transition-colors hover:text-white"
              >
                {t(link.key)}
              </a>
            ))}
          ...
          <div className="hidden items-center gap-4 lg:flex">
            <div className="relative">
              ...
              {langOpen && (
                <div className="absolute right-0 mt-1 w-24 rounded-md border border-charcoal-800 bg-charcoal-950 py-1 shadow-lg">
                  {languages.map((lang) => (
                    <button
                      key={lang.code}
                      type="button"
      ...
      {mobileOpen && (
        <div className="fixed inset-0 z-50 flex flex-col bg-charcoal-950 lg:hidden">
          <div className="flex h-20 items-center justify-between px-4 sm:px-6">
            <a href="#" onClick={closeMobile} className="flex items-center font-['DM_Sans'] text-xl font-bold tracking-wider">
              <span className="text-silver-300">CALVIA</span>
              <span className="mx-0.5 text-amber-500">.</span>
              <span className="text-silver-300">CLUB</span>
            </a>
          ...
          <div className="flex flex-1 flex-col items-center justify-center gap-8">
            {navLinks.map((link) => (
              <a key={link.key} href={link.href} onClick={closeMobile} className="text-2xl font-medium text-silver-300 hover:text-white">
                {t(link.key)}
              </a>
            ))}
